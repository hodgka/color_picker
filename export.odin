...
palette_to_hex_array :: proc(colors: []rl.Color) -> string {
	if len(colors) == 0 do return "[]"

	buf: [MAX_PALETTE * 14 + 4]u8
	offset := 0
	...
	for c, i in colors {
		if i > 0 {
			s2 := fmt.bprintf(buf[offset:], ", ")
			offset += len(s2)
		}
		s3 := fmt.bprintf(buf[offset:], "\"#%02X%02X%02X\"", c.r, c.g, c.b)
		offset += len(s3)
	}

	s4 := fmt.bprintf(buf[offset:], "]")
	offset += len(s4)
...
export_css :: proc(colors: []rl.Color) -> string {
...
export_tailwind :: proc(colors: []rl.Color) -> string {
...
export_json :: proc(colors: []rl.Color) -> string {
...
export_ase :: proc(colors: []rl.Color, path: string) {
	...
	write_u8 :: proc(b: []u8, offset: ^int, val: u8) {
		b[offset^] = val
		offset^ += 1
	}
	...
	off := 0
	write_be32(buf[:], &off, 0x41534546) // "ASEF"
	write_be16(buf[:], &off, 1) // version major
	write_be16(buf[:], &off, 0) // version minor
	write_be32(buf[:], &off, u32(n)) // number of blocks

	for c in colors {
		write_be16(buf[:], &off, 0x0001) // color entry
		write_be32(buf[:], &off, 22) // block length
		write_be16(buf[:], &off, 1) // name length (1 null char)
		write_be16(buf[:], &off, 0) // null name
		write_be32(buf[:], &off, 0x52474220) // "RGB "
		write_be_f32(buf[:], &off, f32(c.r) / 255.0)
		write_be_f32(buf[:], &off, f32(c.g) / 255.0)
		write_be_f32(buf[:], &off, f32(c.b) / 255.0)
		write_be16(buf[:], &off, 0) // color type: global
	}
