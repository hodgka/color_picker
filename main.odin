...
main :: proc() {
	...
	// TODO: re-enable .WINDOW_RESIZABLE after layout tuning
	// Hardcoded for layout tuning -- cycle through: 700x500, 960x780, 1200x900
	TUNING_W :: 700
	TUNING_H :: 500
	rl.InitWindow(TUNING_W, TUNING_H, "Color Picker")
	defer rl.CloseWindow()
	rl.SetTargetFPS(60)

	if settings.valid {
		rl.SetWindowPosition(settings.window_x, settings.window_y)
	}

	icon := rl.LoadImage("dropper_icon.png")
	if icon.data != nil {
	...
	defer {
		exit_settings := capture_settings(&app)
		save_settings(&exit_settings)
	...
	for !rl.WindowShouldClose() {
		...
		app.layout = compute_layout(i32(rl.GetScreenWidth()), i32(rl.GetScreenHeight()))
		L := app.layout
		...
		if app.eyedropper_active {
			...
			rl.DrawText("Click to pick color, ESC to cancel", 10, L.win_h - 24, 14, rl.WHITE)
			rl.EndDrawing()
		...
		if !app.export_open {
			if app.picker_mode == .Square {
				sv_rect := rl.Rectangle{f32(L.sv_x), f32(L.sv_y), f32(L.sv_size), f32(L.sv_size)}
				hue_y := L.sv_y + L.sv_size + 14
				hue_rect := rl.Rectangle{f32(L.sv_x), f32(hue_y), f32(L.sv_size), HUE_BAR_H}
				...
				if app.drags.sv {
					ns := clamp((mouse.x - f32(L.sv_x)) / f32(L.sv_size), 0, 1)
					nv := 1 - clamp((mouse.y - f32(L.sv_y)) / f32(L.sv_size), 0, 1)
					color_set_hsv(&app.cs, app.cs.hue, ns, nv)
				}
				if app.drags.hue {
					nh := clamp((mouse.x - f32(L.sv_x)) / f32(L.sv_size) * 360, 0, 359.99)
			...
			// ── RGB slider input ──
			for i in 0 ..< 3 {
				sy := f32(L.slider_base_y) + f32(i) * f32(L.slider_spacing)
			...
			// ── HSV slider input ──
			for i in 0 ..< 3 {
				sy := f32(L.hsv_slider_y) + f32(i) * f32(L.slider_spacing)
				sr := rl.Rectangle{slider_x, sy, slider_w, 16}
				if rl.IsMouseButtonPressed(.LEFT) && rl.CheckCollisionPointRec(mouse, sr) {
			...
			for i in 0 ..< app.palette_count {
				if rl.CheckCollisionPointRec(mouse, swatch_rect(i, palette_base_y, L.cols_per_row)) {
					app.palette_hover = i
					break
				}
			}
		...
		// ── Draw ──
		draw_frame(&app, mouse, color, display_color)
	}
}
